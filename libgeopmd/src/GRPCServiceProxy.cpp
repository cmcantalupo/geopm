...
namespace geopm
{
    ...
    GRPCServiceProxy::GRPCServiceProxy()
        : m_grpc_socket("unix:///run/geopm/grpc.sock")
        , m_pidfd(-1)
    {
        if (access(m_grpc_socket.c_str(), R_OK|W_OK) != 0) {
            throw geopm::Exception("GRPCServiceProxy: Failed to connect with gRPC socket",
