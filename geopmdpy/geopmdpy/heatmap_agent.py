...
# The derived IPC rows are inserted directly beneath the CPU power rows (and
# above CPU scalability) rather than appended after the GPU rows, keeping the
# CPU-related rows grouped together.
_IPC_INSERT_AFTER_LABEL = _CANDIDATE_SIGNALS[0][2]
...
class HeatmapAgent(Agent):
    ...
    def update_loop(self):
        ...
        self._rows.append([pio.sample(idx) for idx in self._signal_idx])
        if self._instr_idx:
...
def _load_trace(csv_path):
    ...
    rows = df[cols].astype(float).values.tolist()
...
def _render_heatmap(times, labels, rows, ipc_rows, fom_path, out_path):
    ...
    if ipc_rows:
        ipc_labels = []
        ipc_matrix = []
        for ipc_label, ipc_series in ipc_rows:
            ...
            ipc_labels.append(ipc_label)
            ipc_matrix.append(ipc_row)
        # Insert the IPC rows just below the CPU power rows (above CPU
        # scalability) so the CPU-related rows stay grouped together.
        insert_at = sum(1 for lbl in labels
                        if lbl == _IPC_INSERT_AFTER_LABEL
                        or lbl.startswith(_IPC_INSERT_AFTER_LABEL + ' '))
        labels = labels[:insert_at] + ipc_labels + labels[insert_at:]
        matrix = np.vstack([matrix[:insert_at]] + ipc_matrix
                           + [matrix[insert_at:]])
    ...
    # Keep each heatmap row thin so many-socket/many-GPU nodes do not
    # produce an excessively tall figure, and cap the overall height.
    heat_height = min(0.25 * n_rows + 1.0, 16.0)

    have_fom = fom_path is not None
    if have_fom:
        fig, (ax, ax_fom) = plt.subplots(
            2, 1, figsize=(12, heat_height + 1.5), sharex=True,
            gridspec_kw={'height_ratios': [max(n_rows, 1), 2]})
    else:
        fig, ax = plt.subplots(figsize=(12, heat_height))

    im = ax.imshow(norm, aspect='auto', cmap='magma', interpolation='nearest',
                   extent=[float(times[0]), float(times[-1]), n_rows, 0],
                   vmin=0.0, vmax=1.0)
